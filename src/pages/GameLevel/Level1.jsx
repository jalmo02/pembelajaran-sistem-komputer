...
// ── LottieAnim: pakai lottie-web langsung ────────────────────────────────────
...
const STORAGE_HISTORY = "game_history_level1";
const STORAGE_LEVEL1_DONE = "game_level1_ever_done";
const LEVEL = 1;
...
export default function Level1() {
  ...
  // ── Lightning — pakai LottiePreloaded + lightningRef (sama kayak Level 5) ──
  ...
  // ── Floating panel toggles (sama kayak Level 5) ────────────────────────────
  ...
  const initPuzzle = useCallback(() => {
    ...
    if (petirAnimRef.current) {
      petirAnimRef.current.stop();
      petirAnimRef.current.goToAndStop(0, true);
    }
  ...
  const saveScore = useCallback(
    (score, currentAttempt) => {
      ...
      const alreadyDone = getUserItem(STORAGE_LEVEL1_DONE, false);
      if (!alreadyDone) {
        setUserItem(STORAGE_LEVEL1_DONE, true);
  ...
  const onDropBoard = useCallback(
    (e, si) => {
      ...
      if (correctOrder) {
        ...
        // Pakai LottiePreloaded + lightningRef (sama kayak Level 5)
    ...
    <>
      <style>{`
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        /* Side panel — hanya untuk konten internal HistoryPanel & InfoPanel */
        .side-panel { width:100%; display:flex; flex-direction:column; overflow:hidden; height:100%; }
        ...
        /* Lightning board overlay — pre-mount, sama kayak Level 5 */
      ...
      <div className="lv">
        ...
        <header className="lv-hdr">
          ...
          <div className="lv-hdr-right">
            ...
            <div className="lv-badge">LVL 1</div>
          </div>
        </header>
        ...
        <div className="lv-body">
          ...
          {/* ICON JAM — toggle riwayat (floating, sama kayak Level 5) */}
          <button
            onClick={() => setShowHistory((h) => !h)}
            style={{
              position: "absolute", top: 10, left: 10, zIndex: 20,
              width: 36, height: 36,
              background: showHistory ? "rgba(180,0,255,.25)" : "rgba(10,2,25,.7)",
          ...
          >
            <svg
              ...
              stroke={showHistory ? "#e879f9" : "rgba(180,0,255,.6)"} strokeWidth="2"
            >
              <circle cx="12" cy="12" r="10" />
              <polyline points="12 6 12 12 16 14" />
            </svg>
          ...
          {/* PANEL RIWAYAT — floating overlay */}
          {showHistory && (
            <div style={{
              ...
              <div className="lv-main">
                ...
                <div className="lv-board-wrap">
                  ...
                  >
                    ...
                    {/* Pre-mounted lightning overlay — sama kayak Level 5 */}
                    <div
                ...
                >
                  <div
                    style={{
                      height: 1,
                      flex: 1,
                      background: "linear-gradient(90deg,rgba(180,0,255,.4),transparent)",
                    }}
                  />
                  <p
                    style={{
                  ...
                  <div
                    style={{
                      height: 1,
                      flex: 1,
                      background: "linear-gradient(90deg,transparent,rgba(180,0,255,.4))",
                    }}
                  />
                </div>
          ...
          {/* ICON TANDA TANYA — toggle info (floating, sama kayak Level 5) */}
          <button
            onClick={() => setShowInfo((i) => !i)}
            style={{
              position: "absolute", top: 10, right: 10, zIndex: 20,
              width: 36, height: 36,
              background: showInfo ? "rgba(180,0,255,.25)" : "rgba(10,2,25,.7)",
              border: "1px solid rgba(180,0,255,.4)", cursor: "pointer",
              display: "flex", alignItems: "center", justifyContent: "center",
              transition: "all .2s",
            }}
